...
export const sortDrivers = drivers.sort((b, a) => {
...
// criando base de times  usando o reduce
export const generateTeamsArray = () => sortDrivers.reduce(
  (acc, currentValueTeam) => {
    const {team, points} = currentValueTeam; // pegando o time e os pontos
    const teamObject = acc.find(t => t.team === team) // se o time ja existe
    teamObject ? (teamObject.points += points) : acc.push({team: team, points: points}); // adicionando o time e os pontos
    return acc; // retornando o acumulador
  }, []).sort((b, a) => {
    if (a.points > b.points) {
        return 1;
    }
